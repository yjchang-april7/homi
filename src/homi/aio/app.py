...
async def AsyncNotImplementedMethod(request, context):
...
class BaseAsyncService(Generic[ConfigType], ABC):
    def __init__(
            self,
    ...
    def register_app(self, app):
        ...
        if self._after_registered_handler:
            ...
            # todo: if register app can request async remove annotation
            # if inspect.isawaitable(self._after_registered_handler):
            #     self._after_registered_handler(self)
            # else:
            #     self._after_registered_handler(self)
    ...
    def before_server_start_handler(self):
        if self._before_server_start_handler:
            ...
            # todo: if register app can request async remove annotation
            # if inspect.isawaitable(self._before_server_start_handler):
            #     await self._before_server_start_handler(self)
            # else:
            #     self._before_server_start_handler(self)
    ...
    def make_servicer_class(self):
        ...
        for name, method_meta in self.meta.methods.items():
            if name in self._method_handler:
                func = warp_async_handler(method_meta, self._method_handler[name])
            else:
                func = AsyncNotImplementedMethod
            methods[name] = func
...
class AsyncApp(BaseAsyncApp):
    def __init__(self, services: List[Union[AsyncService, ServiceDescriptor]] = None, **kwargs):
    ...
    def get_service_by_full_name(self, full_name: str) -> AsyncService:
        try:
            return self._services[full_name]
        except KeyError:
            raise ServiceNotFound(full_name, available_services=self.service_names)

    def add_service(self, service: AsyncService):
        if not isinstance(service, BaseAsyncService) and not isinstance(service, BaseService):
    ...
    def add_service_from_descriptor(self, descriptor: ServiceDescriptor):
        self.add_service(AsyncService(descriptor))
